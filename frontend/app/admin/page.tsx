// PAINEL ADMIN — CAPI DA SORTE — PARTE 1
'use client'
...
const TEMAS = [
  ...
  { id:'aniversario', nome:'Aniversariante', cor:'#FF8C00', elementos:'Balões coloridos subindo • Frase impactante • Caixa dourada especial • Desconto exclusivo' },
]

type Menu = 'dashboard'|'criar_campanha'|'campanhas_ativas'|'historico_campanhas'|'sorteios'|'pacotes'|'caixas'|'extrato'|'relatorio_premios'|'simulador'|'usuarios'|'blacklist'|'templates_whatsapp'|'gatilhos'|'identidade'|'temas'|'textos'|'aniversariantes'|'logs'|'controle_acesso'
...
const MENU_ITEMS = [
  ...
  { section:'COMUNICAÇÃO', items:[
    { id:'templates_whatsapp', label:'Templates WhatsApp' },
...
export default function Admin() {
  ...
  const [novaCamp, setNovaCamp] = useState({
    nome:'', total_cotas:'1000000', valor_cota:'4.99',
    premio_inicial:'100000', incremento_por_cota:'1.50',
    porcentagem_premio:'30', limite_cotas_por_cpf:'200',
    tempo_reserva_minutos:'30',
    data_sorteio1:'', data_sorteio2:'', data_sorteio3:'', data_sorteio_principal:'',
    premio_sorteio1:'500', premio_sorteio2:'800', premio_sorteio3:'1200',
  })
  ...
  const [sorteios, setSorteios] = useState([
    { ordem:'1º Sorteio', data:'2026-04-15', hora:'20:00', premio:'500', status:'realizado', ganhador:'', bilhete_ganhador:'' },
    { ordem:'2º Sorteio', data:'2026-04-20', hora:'20:00', premio:'800', status:'realizado', ganhador:'', bilhete_ganhador:'' },
    { ordem:'3º Sorteio', data:'2026-04-26', hora:'20:00', premio:'1200', status:'pendente', ganhador:'', bilhete_ganhador:'' },
    { ordem:'Sorteio Principal', data:'2026-04-30', hora:'20:00', premio:'', status:'pendente', ganhador:'', bilhete_ganhador:'' },
  ])
  ...
  const [templates, setTemplates] = useState({
    boas_vindas:'Olá {nome}! Bem-vindo à Capi da Sorte! Seus {quantidade} bilhetes foram confirmados. Seus números: {numeros}. Boa sorte!',
    confirmacao:'Pagamento confirmado! Seus {quantidade} bilhetes estão garantidos. O prêmio já está em R$ {premio}. Boa sorte!',
    lembrete_sorteio:'O {sorteio} acontece em breve! Você tem {quantidade} bilhetes participando. Garanta mais: {link}',
    recompra:'Olá {nome}! O prêmio já está em R$ {premio}. Garanta mais bilhetes: {link}',
    ganhador:'Parabéns {nome}! Você ganhou R$ {premio} no {sorteio} da Capi da Sorte!',
    aniversario:'Feliz Aniversário {nome}! A Capi preparou uma surpresa especial para você hoje! Acesse: {link}',
    congelamento:'Atenção {nome}! A campanha foi encerrada. Prêmio acumulado: R$ {premio}. O sorteio acontece em breve. Boa sorte!',
  })

  const [gatilhos, setGatilhos] = useState([
    { id:1, nome:'Boas-vindas após compra', ativo:true, tempo:'Imediato', template:'boas_vindas' },
    { id:2, nome:'Lembrete 24h antes do sorteio', ativo:true, tempo:'24h antes', template:'lembrete_sorteio' },
    { id:3, nome:'Recompra 3 dias sem comprar', ativo:true, tempo:'72h inativo', template:'recompra' },
    { id:4, nome:'Aniversário do cliente', ativo:true, tempo:'Dia do aniversário', template:'aniversario' },
    { id:5, nome:'Congelamento de campanha', ativo:true, tempo:'Automático', template:'congelamento' },
  ])
  ...
  const simTotal = parseFloat(simBilhetes)||0
  const simVal = parseFloat(simValor)||0
  const simPctN = parseFloat(simPct)||0
  const simPctVN = parseFloat(simPctVenda)||0
  const simBilhetesVendidos = Math.floor(simTotal * simPctVN/100)
  const simReceitaBruta = simBilhetesVendidos * simVal
  const simPremioFinal = simReceitaBruta * simPctN/100
  ...
  const aniversariantesMes = usuarios.filter(u => {
    ...
    return new Date(u.data_nascimento).getMonth() === new Date().getMonth()
  })
    ...
    <>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        body{background:#04091C;overflow-x:hidden}
          ...
          <div style={{padding:'24px 20px 80px',maxWidth:900,margin:'0 auto'}}>
            ...
            {menu==='criar_campanha'&&(
              <div>
                ...
                <div className="card">
                  <div style={{fontFamily:"'Bebas Neue',cursive",fontSize:16,color:'#F5A800',letterSpacing:2,marginBottom:16}}>Informações Básicas</div>
                  <div style={{marginBottom:12}}>
                    <div className="lbl">Nome da Campanha</div>
                    <input className="inp" placeholder="Ex: Campanha Maio 2026" value={novaCamp.nome} onChange={e=>setNovaCamp(p=>({...p,nome:e.target.value}))}/>
                  </div>
                ...
                <div className="card">
                  <div style={{fontFamily:"'Bebas Neue',cursive",fontSize:16,color:'#F5A800',letterSpacing:2,marginBottom:16}}>Porcentagem do Prêmio</div>
                  <div className="lbl">Quanto da receita vai para o prêmio?</div>
                ...
                <div className="card">
                  <div style={{fontFamily:"'Bebas Neue',cursive",fontSize:16,color:'#F5A800',letterSpacing:2,marginBottom:16}}>Datas dos Sorteios</div>
                  <div className="grid2" style={{marginBottom:12}}>
                    <div><div className="lbl">1º Sorteio — Data</div><input className="inp" type="datetime-local" value={novaCamp.data_sorteio1} onChange={e=>setNovaCamp(p=>({...p,data_sorteio1:e.target.value}))}/></div>
                    <div><div className="lbl">1º Sorteio — Prêmio (R$)</div><input className="inp" type="number" value={novaCamp.premio_sorteio1} onChange={e=>setNovaCamp(p=>({...p,premio_sorteio1:e.target.value}))}/></div>
                  </div>
                  <div className="grid2" style={{marginBottom:12}}>
                    <div><div className="lbl">2º Sorteio — Data</div><input className="inp" type="datetime-local" value={novaCamp.data_sorteio2} onChange={e=>setNovaCamp(p=>({...p,data_sorteio2:e.target.value}))}/></div>
                    <div><div className="lbl">2º Sorteio — Prêmio (R$)</div><input className="inp" type="number" value={novaCamp.premio_sorteio2} onChange={e=>setNovaCamp(p=>({...p,premio_sorteio2:e.target.value}))}/></div>
                  </div>
                  <div className="grid2" style={{marginBottom:12}}>
                    <div><div className="lbl">3º Sorteio — Data</div><input className="inp" type="datetime-local" value={novaCamp.data_sorteio3} onChange={e=>setNovaCamp(p=>({...p,data_sorteio3:e.target.value}))}/></div>
                    <div><div className="lbl">3º Sorteio — Prêmio (R$)</div><input className="inp" type="number" value={novaCamp.premio_sorteio3} onChange={e=>setNovaCamp(p=>({...p,premio_sorteio3:e.target.value}))}/></div>
                  </div>
                  <div><div className="lbl">Sorteio Principal — Data</div><input className="inp" type="datetime-local" value={novaCamp.data_sorteio_principal} onChange={e=>setNovaCamp(p=>({...p,data_sorteio_principal:e.target.value}))}/></div>
                </div>
                <button className="btn-g" style={{width:'100%',padding:16,fontSize:15}} onClick={()=>salvar('Campanha criada com sucesso!')}>Criar Campanha</button>
              </div>
            )}
                ...
                {campanha?(
                  <div className="card">
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20,flexWrap:'wrap',gap:12}}>
                      ...
                      <button className="btn-r" onClick={()=>salvar('Campanha congelada! Mensagens disparadas para todos os participantes.')}>Congelar Campanha</button>
                    </div>
                    <div className="grid2" style={{marginBottom:16}}>
                      {[
                        {lbl:'Premio Atual',val:`R$ ${fv(premioAtual)}`},
                    ...
                    <div style={{display:'flex',justifyContent:'space-between',fontSize:12,color:'#7A8BB0',marginTop:6}}>
                      ...
                      <span>{(campanha.total_cotas-campanha.cotas_vendidas).toLocaleString('pt-BR')} disponíveis</span>
                    </div>
                  </div>
                ...
                {sorteios.map((s,i)=>(
                  <div key={i} className="card">
                    ...
                    <div className="grid2" style={{marginBottom:12}}>
                      ...
                      <div><div className="lbl">Bilhete Ganhador</div><input className="inp" placeholder="Número do bilhete" value={s.bilhete_ganhador} onChange={e=>{const n=[...sorteios];n[i].bilhete_ganhador=e.target.value;setSorteios(n)}}/></div>
                    </div>
                    <div style={{marginBottom:12}}>
                      <div className="lbl">Nome do Ganhador</div>
                      <input className="inp" placeholder="Nome do ganhador" value={s.ganhador} onChange={e=>{const n=[...sorteios];n[i].ganhador=e.target.value;setSorteios(n)}}/>
                    </div>
            ...
            {menu==='caixas'&&(
              <div>
                ...
                <div style={{fontSize:13,color:'#7A8BB0',marginBottom:16}}>Uma caixa por compra. O cliente clica para revelar. O sistema distribui automaticamente.</div>
            ...
            {menu==='simulador'&&(
              <div>
                ...
                <div style={{fontSize:13,color:'#7A8BB0',marginBottom:20}}>Simule o resultado financeiro antes de abrir as vendas.</div>
                <div className="card" style={{marginBottom:16}}>
                  ...
                  <div className="grid2" style={{marginBottom:12}}>
                    ...
                    <div><div className="lbl">% de Venda Estimada</div>
            ...
            {menu==='usuarios'&&(
              <div>
                ...
                <input className="inp" style={{marginBottom:16}} placeholder="Buscar por nome ou email..." value={busca} onChange={e=>setBusca(e.target.value)}/>
            ...
            {menu==='aniversariantes'&&(
              <div>
                ...
                <div className="card" style={{marginBottom:16}}>
                  <div style={{fontFamily:"'Bebas Neue',cursive",fontSize:16,color:'#F5A800',letterSpacing:2,marginBottom:12}}>Presente de Aniversário</div>
                  <div className="grid2" style={{marginBottom:12}}>
                    <div><div className="lbl">Desconto Exclusivo (%)</div><input className="inp" type="number" defaultValue="20"/></div>
                    <div><div className="lbl">Validade (horas)</div><input className="inp" type="number" defaultValue="24"/></div>
                  </div>
                  <button className="btn-g" onClick={()=>salvar('Configuração salva!')}>Salvar</button>
                </div>
            ...
            {menu==='blacklist'&&(
              <div>
                ...
                <div className="card" style={{marginBottom:16}}>
                  ...
                  <div className="grid2" style={{marginBottom:12}}>
                    <div><div className="lbl">CPF ou Email</div><input className="inp" placeholder="000.000.000-00 ou email@email.com"/></div>
                    <div><div className="lbl">Motivo</div><input className="inp" placeholder="Motivo do bloqueio"/></div>
                  </div>
                  <button className="btn-r" onClick={()=>salvar('Adicionado à blacklist!')}>Bloquear</button>
                </div>
                <div style={{textAlign:'center',padding:40,color:'#7A8BB0',fontSize:13}}>Nenhum registro na blacklist</div>
              </div>
            )}

            {/* TEMPLATES WHATSAPP */}
            {menu==='templates_whatsapp'&&(
              <div>
                <div className="sec-title">Templates WhatsApp</div>
                <div style={{fontSize:13,color:'#7A8BB0',marginBottom:16}}>Use {'{nome}'}, {'{quantidade}'}, {'{premio}'}, {'{numeros}'}, {'{link}'}, {'{sorteio}'} como variáveis.</div>
                {Object.entries(templates).map(([key,val])=>(
                  <div key={key} className="card">
                    <div className="lbl">{key.replace(/_/g,' ')}</div>
                    <textarea className="textarea" style={{marginTop:8}} value={val} onChange={e=>setTemplates(p=>({...p,[key]:e.target.value}))}/>
                    <button className="btn-s" style={{marginTop:8}} onClick={()=>salvar('Template salvo!')}>Salvar</button>
                  </div>
                ))}
              </div>
            )}
            ...
            {menu==='gatilhos'&&(
              <div>
                ...
                <div style={{fontSize:13,color:'#7A8BB0',marginBottom:16}}>Configure quando cada mensagem é disparada automaticamente.</div>
                {gatilhos.map((g,i)=>(
                  <div key={i} className="card">
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:12}}>
                      <div>
                        <div style={{fontSize:14,fontWeight:700,color:'#fff'}}>{g.nome}</div>
                        <div style={{fontSize:12,color:'#7A8BB0',marginTop:2}}>Disparo: {g.tempo}</div>
                        <div style={{fontSize:12,color:'#4A5B7A',marginTop:2}}>Template: {g.template}</div>
                      </div>
                      <div style={{display:'flex',alignItems:'center',gap:12}}>
                        <span style={{fontSize:12,color:g.ativo?'#1FCC6A':'#7A8BB0'}}>{g.ativo?'Ativo':'Inativo'}</span>
                        <button className="toggle" style={{background:g.ativo?'#1FCC6A':'rgba(255,255,255,0.1)'}} onClick={()=>{const n=[...gatilhos];n[i].ativo=!n[i].ativo;setGatilhos(n)}}>
              ...
              <div>
                ...
                <div className="card">
                  <div className="grid2" style={{marginBottom:16}}>
                    ...
                    <div>
                      <div className="lbl">Cor Principal</div>
                    ...
                    <div>
                      <div className="lbl">Cor de Fundo</div>
                      <div style={{display:'flex',gap:8,alignItems:'center',marginTop:4}}>
